...
function checkNews(since) {
	getNews(since).then(function (data) {
...
function displayNews() {
	setInterval(function () {
		if (newsPool.length > 0) {
			...
			var buffer = 200;
			var left = Math.floor(Math.random() * (rect.width + buffer) - buffer / 2);
			var top = Math.floor(Math.random() * (rect.height + buffer) - buffer / 2);
...
function updateNews() {
	...
	setInterval(function () {
		document.querySelectorAll('.news-item').forEach(function (newsItem) {
			var now = new moment().format('X') * 1000 + new Date().getMilliseconds();
			var time = parseInt(newsItem.dataset.time);
			var p = (now - time) / displayTime;
